...
export default async (req) => {
  ...
  if (!childName) return json(400, { error: "Missing childName" });
  ...
  try {
    const payload = [{ profile_code: profileCode, child_name: childName }];
    const resp = await fetch(`${url}/rest/v1/profiles?on_conflict=profile_code`, {
      method: "POST",
      headers: {
